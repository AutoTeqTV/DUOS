...
function HudClass(Nav, c, u, s, atlas, antigrav, hover, shield, warpdrive, weapon,
    ...
    --Local Huds Functions
        ...
        local function DrawWarnings(newContent)
            ...
            newContent[#newContent + 1] = svgText(crx(10), cry(1048), "DUOS // FLIGHT OPERATIONS", "pbright txtstart size14")
            newContent[#newContent + 1] = svgText(crx(10), cry(1068), stringf("DUOS FlightHUD v0.3 | ARCHHUD CORE %.3f | FLIGHT CORE ONLINE", VERSION_NUMBER), "pdim txtstart size14")
            ...
            if BrakeIsOn then
                local bkStr = ""
                if type(BrakeIsOn) == "string" then bkStr=" ["..string.upper(tostring(BrakeIsOn)).."]" end
                newContent[#newContent + 1] = svgText(warningX, brakeY, "BRAKE: ENGAGED"..bkStr, "warnings")
            elseif brakeInput2 > 0 then
                newContent[#newContent + 1] = svgText(warningX, brakeY, "BRAKE: AUTO", "warnings", "opacity:"..brakeInput2)
            end
            ...
            if inAtmo and stalling and abvGndDet == -1 then
                if not Autopilot and not VectorToTarget and not BrakeLanding and not antigravOn and not VertTakeOff and not AutoTakeoff then
                    newContent[#newContent + 1] = svgText(warningX, apY+50, "ALERT: STALL", "warnings")
                    play("stall","SW",2)
                end
            end
            if ReversalIsOn then
                newContent[#newContent + 1] = svgText(warningX, apY+90, "ASSIST: ACTIVE", "warnings")
            end
            ...
            if GearExtended then
                ...
                if hasGear then
                    ...
                    newContent[#newContent + 1] = svgText(warningX, gearY, "LANDING: GROUNDED [G TAKEOFF]", "warnings")
                end
            end
            if abvGndDet > -1 and (not antigravOn or coreAltitude < 100) then 
                ...
                newContent[#newContent + 1] = svgText(warningX, hoverY,"HOVER: ".. displayText,"warn") 
            end

            if isBoosting then

                newContent[#newContent + 1] = svgText(warningX, ewarpY+20, "BOOST: ROCKET ACTIVE", "warn")
            end           
            ...
            if antigrav and not ExternalAGG and antigravOn and AntigravTargetAltitude ~= nil then
                ...
                if mabs(coreAltitude - antigrav.getBaseAltitude()) < 501 then aggWarn = "warn" end
                    newContent[#newContent + 1] = svgText(warningX, apY+40, stringf("AGG TARGET: %d | SINGULARITY: %d", mfloor(AntigravTargetAltitude), mfloor(antigrav.getBaseAltitude())), aggWarn)
            end
            if Autopilot and AutopilotTargetName ~= "None" then
                newContent[#newContent + 1] = svgText(warningX, apY,  "AP: "..AutopilotStatus, "warn")
            elseif LockPitch ~= nil then
                newContent[#newContent + 1] = svgText(warningX, apY+20, stringf("PITCH LOCK: %d", mfloor(LockPitch)), "warn")
            elseif followMode then
                newContent[#newContent + 1] = svgText(warningX, apY+20, "FOLLOW: ACTIVE", "warn")
            elseif Reentry or finalLand then
                newContent[#newContent + 1] = svgText(warningX, apY+20, "RE-ENTRY: ACTIVE", "warn")
            end
            if AltitudeHold or VertTakeOff then
                ...
                if VertTakeOff then
                    ...
                    newContent[#newContent + 1] = svgText(warningX, apY, "VTO: "..displayText , "warn")
                elseif (AutoTakeoff or spaceLaunch) and not IntoOrbit then
                    if spaceLaunch then
                        newContent[#newContent + 1] = svgText(warningX, apY, "TAKEOFF: "..AutopilotTargetName, "warn")
                    else
                        newContent[#newContent + 1] = svgText(warningX, apY, "TAKEOFF: "..displayText, "warn")
                    end
                    if BrakeIsOn and not VertTakeOff then
                        newContent[#newContent + 1] = svgText( warningX, apY + 80,"TAKEOFF: THROTTLE UP / RELEASE BRAKE", "crit")
                    end
                else
                    newContent[#newContent + 1] = svgText(warningX, apY, "ALT HOLD: ".. stringf("%.1fm", HoldAltitude), "warn")
                end
            end
            if VertTakeOff and (antigrav ~= nil and antigrav) then
                if atmosDensity > 0.1 then
                    newContent[#newContent + 1] = svgText(warningX, apY+20, "VTO: ASCENT", "warn")
                elseif atmosDensity < 0.09 and atmosDensity > 0.05 then
                    newContent[#newContent + 1] = svgText(warningX, apY+20,  "VTO: TRAJECTORY ALIGN", "warn")
                elseif atmosDensity < 0.05 then
                    newContent[#newContent + 1] = svgText(warningX, apY+20,  "VTO: LEAVING ATMOSPHERE", "warn")
                end
            end
            if IntoOrbit then
            ...
            if BrakeLanding then
                    local str = "LANDING: BRAKE"
                    if alignHeading then str = str.." | ALIGN" end
                    if apBrk then str = str.." | DRIFT LIMIT" end
                    newContent[#newContent + 1] = svgText(warningX, apY, str, "warnings")
            end
            if ProgradeIsOn then
                newContent[#newContent + 1] = svgText(warningX, apY+20, "ALIGN: PROGRADE", "crit")
            end
            if RetrogradeIsOn then
                newContent[#newContent + 1] = svgText(warningX, apY, "ALIGN: RETROGRADE", "crit")
            end
        ...
        local function MakeTabButton(x, y, width, height, label, displayLabel)
            local newButton = {
                x = x, 
                y = y,
                width = width,
                height = height,
                label = label,
                displayLabel = displayLabel or label
            }
        ...
        local function ControlsButtons()
            ...
            local button = MakeTabButton(0,0,crx(80),tabHeight, "INFO", "SYS")
            button = MakeTabButton(button.x + button.width,button.y,crx(70),tabHeight,"ORBIT", "NAV")
            button = MakeTabButton(button.x + button.width,button.y,crx(80),tabHeight,"SCOPE", "RADAR")
            MakeTabButton(button.x + button.width,button.y,crx(70),tabHeight,"HIDE", "HIDE")
        end
    ...
    function Hud.OneSecondTick()
        ...
        local function extraData(newContent)
            ...
            if DisplayOdometer then 
                ...
                --TODO: newContent[#newContent + 1] = svgText(crx(700), cry(30), stringf("LIFE: %.2f kSU", (TotalDistanceTravelled / 200000)), "txtstart") 
                ...
                --newContent[#newContent + 1] = svgText(crx(830), cry(10), stringf("BRK DIST: %s", getDistanceDisplayString(brkDist)) , "txtstart")
        ...
        local function DrawTabButtons(newContent)
            ...
            for k,v in pairs(TabButtons) do
                ...
                newContent[#newContent + 1] = svgText(v.x+v.width/2, v.y + v.height/2 + 5, v.displayLabel or v.label, "txt txtmid pdim")
            end
        end
        local function DrawInfo(totalDistanceTrip, TotalDistanceTravelled, flightTime)
            ...
            if not isRemote() or RemoteHud then 
                ...
                local startY = cry(OrbitMapY+42)
                local midX = crx(OrbitMapX+60*1920/ResolutionX+OrbitMapSize/2)
                local height = 25
                infoContent = infoContent.. "<g class='txtstart size14 bright'>"
                infoContent = infoContent.. svgText(startX, cry(OrbitMapY+22), "DUOS TELEMETRY", "pbright txtstart size20")
                infoContent = infoContent.. svgText(startX, startY, stringf("BRK ETA: %s", FormatTimeString(brkTime)))
                infoContent = infoContent.. svgText(midX, startY, stringf("TRIP: %.2f km", totalDistanceTrip)) 
                infoContent = infoContent.. svgText(startX, startY+height, stringf("LIFE: %.2f kSU", (TotalDistanceTravelled / 200000))) 
                infoContent = infoContent.. svgText(midX, startY+ height, stringf("BRK DIST: %s", getDistanceDisplayString(brkDist)))
                ...
                infoContent = infoContent.. svgText(midX, startY+height*4, stringf("SAFE BRAKE: %s",  safeBrakeMass)) 
                infoContent = infoContent.. svgText(startX, startY+height*4, stringf("MAX THRUST: %s", maxThrust)) 
                infoContent = infoContent.. svgText(midX, startY+height*5, stringf("SAFE ATMO: %s", (safeAtmoMass)))
                infoContent = infoContent.. svgText(startX, startY+height*5, stringf("MAX BRAKE: %s", brakeValue )) 
                infoContent = infoContent.. svgText(midX, startY+height*6, stringf("SAFE SPACE: %s", (safeSpaceMass)))
                infoContent = infoContent.. svgText(midX, startY+height*7, stringf("SAFE HOVER: %s", (safeHoverMass)))
                infoContent = infoContent.. svgText(startX, startY+height*6, stringf("INFLUENCE: %s", planet.name))
                infoContent = infoContent.. svgText(startX, startY+height*7, stringf("SET MAX: %s", mfloor(adjMaxGameVelocity*3.6+0.5)))
                infoContent = infoContent.. svgText(midX, startY+height*8, stringf("ACTUAL MAX: %s", mfloor(MaxSpeed*3.6+0.5)))
